...
public class Logic3T {
     ...
     *
     * @return Возвращает true если есть выигрышные комбинации Крестика
     */
     ...
     *
     * @return Возвращает true если есть выигрышные комбинации Нолика
     */
     ...
     *
     * @return Возвращает true если можно сделать ход
     */
    public boolean hasGap() {
        for (Figure3T[] row : table) {
            for (int j = 0; j < table.length; j++) {
                if (!(row[j].hasMarkX() || row[j].hasMarkO())) {
                    return true;
                }
            }
        }
